...
const fetchLiterary = async (center, radiusMiles) => {
  ...
  // Use searchR (trip-radius-based) not CATEGORY_RADII here — compass points are spaced
  // at radiusMiles*0.5 apart, so each query needs radius >= that/2 to avoid blind spots.
  // The day-trip algorithm selects only a few stops from candidates, so more candidates = better.
...
export const generateDayTrip = async (startCoords, duration, variant = 0, excludedIds = new Set(), categories = null, restaurantFilter = null) => {
  ...
  // Use searchR (not CATEGORY_RADII) so compass-sampled points cover the full trip radius
  // without blind spots. Day trips select only a few stops from candidates anyway.
