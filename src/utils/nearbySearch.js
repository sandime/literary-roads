// Combined nearby search: Firestore bookstores + Foursquare cafes/drive-ins
// Drop-in replacement for googlePlaces.searchNearbyPlaces + searchAlongRoute
import { getNearbyBookstores } from './firestorePlaces';
import { searchNearbyCafes, searchNearbyDriveIns } from './foursquare';
...
// Combines: Firestore bookstores (free) + Foursquare cafes + Foursquare drive-ins
export const searchNearbyPlaces = async (lat, lng, radiusMiles = 5) => {
  const [bookstores, cafes, driveIns] = await Promise.all([
    ...
    searchNearbyCafes(lat, lng, radiusMiles),
