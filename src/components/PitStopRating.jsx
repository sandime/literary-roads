...
export default function PitStopRating({ locationId, user, onShowLogin, onStarburstChange }) {
  ...
  const handleVote = async (vote) => {
    ...
    setError('');
    try {
      await castVote(locationId, user.uid, vote);
    } catch (err) {
        ...
        <div className="flex items-center justify-between">
          ...
          {yesCount > 0 && (
            <span className="text-chrome-silver/50 font-special-elite text-xs">
              {yesCount} traveler{yesCount !== 1 ? 's' : ''} agree
            </span>
          )}
        </div>
      ) : (
            ...
            {user ? (
              <>
                <button
                  ...
                  disabled={submitting}
                  className="flex items-center gap-1.5 border-2 border-starlight-turquoise/60 text-starlight-turquoise font-bungee px-3 py-1 rounded-lg hover:bg-starlight-turquoise hover:text-midnight-navy disabled:opacity-40 transition-all"
                  style={{ fontSize: '11px' }}
                >
                  👍 YES
                </button>
                <button
                  ...
                  disabled={submitting}
                  className="flex items-center gap-1.5 border-2 border-white/20 text-chrome-silver font-bungee px-3 py-1 rounded-lg hover:bg-white/10 disabled:opacity-40 transition-all"
                  style={{ fontSize: '11px' }}
                >
                  👎 NO
                </button>
                {yesCount > 0 && (
                  <span className="text-chrome-silver/40 font-special-elite text-xs ml-1">
                    {yesCount} recommend
                  </span>
                )}
              </>
